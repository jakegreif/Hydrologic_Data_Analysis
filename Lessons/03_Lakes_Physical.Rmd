...
- Present in a depression, typically isolated from other large bodies of water (i.e. the ocean)
- Low or no flow
- Emeregent vegetation possible on the fringes, but not across the entire lake (compared to a wetland)
- Generally, light doesn't penetrate to the deepest point

What are the physical properties of lakes?
- Temperature gradients
- Size threshold (small lakes can be considered ponds)
- Low or no flow, still water (lentic)
- 
...
# A: Summary would give us a statistical summary of things (mean, max, min, etc.) because the class is numeric
```
...
A: Temps are warmer at the surface, and decrease with depth. Temperature also varies more at the surface, and the variance decreases with depth. The bottom of the lake does not interact with the atmosphere.
...
Hypolimneon is the 9-12m where the water is mixed and not changing.
...
#  4-6m are associated with the thermocline, where the temp changes quickly. 
```

Why can't we use a line graph here?
A: We removed the rows with NAs (which would place breaks in the lines), and samples were collected only during the summer.
```{r}
Tempplot.line <- 
  ggplot(Pauldata.surface, aes(x = sampledate, y = temperature_C)) +
...
# the above command flips our graph
