...
> Both can give you an indication of where the water is sourced from, or where pollutants are coming from. There are also ranges of these measures that are adequate for certain uses.
...
# structure function shows us the structure of the data
...
# spread function- key tells it what column it will end up in, and value tells it what value to give those parameters
...
> Kanawha has a higher pH and lower conductance after 1970

Why might mining be causing the changes you see?

> Mining adds a lot of chemicals and ions to the water.
...
> The data was collected infrequently, so it's hard to see the true variability of the data
...
> K river pH stabalizes around 7.3-7.4, and T river pH drops by nearly 1 pH. Conductance in the K river stabalizes as well, and it remains lower than it was in the 60s. T river is a little less variable after 1977.
...
# uses geom density when we're working with just a few points
  ggtitle("Annual Averages of Specific Conductance in the Kanawha River 1958:1997") +
  labs(x = expression("Specific Conductance ("*mu*S * cm**-1*")"), 
...
> Use the density plot to show that SC has decreased, and mention that it's statistically significant.

What can you say with data visualization that you can't say with just the statistical tests?

> The test tells us that there's a difference, but the graph shows you how they're different.
...
Mn.plot <- ggplot(Manganese.dat, aes(x = Date, y = Manganese_ugL)) +
  geom_smooth() +
  geom_point() +
  geom_vline(xintercept = 1977, color = "grey", alpha = 0.8, lwd = 2, lty = 2) + 
  geom_hline(yintercept = 50, color = "grey", alpha = 0.8, lwd = 2, lty = 2) +
  labs(y = expression("Manganese ("*mu*g*l^-1*")")) +
  ggtitle("Manganese Concentrations in the Kanawha River")
...
> Lines b/c it's easier to read

EPA drinking water standards for Manganese are 0.05 *_mg/L_*. How does the Kanawha stack up?

> 0.05 mg/L = 50 ug/L, so the K river was getting close by the end of the data collection period
...
Nitrate.dat <- Nitrate.raw %>%
 select(Date = sample_dt, Nitrate_mgL = result_va)
  
 
N.plot <- ggplot(Nitrate.dat, aes(x = Date, y = Nitrate_mgL)) +
  geom_smooth() +
  geom_point() +
  geom_vline(xintercept = 1977, color = "grey", alpha = 0.8, lwd = 2, lty = 2) + 
  geom_hline(yintercept = 10, color = "grey", alpha = 0.8, lwd = 2, lty = 2) +
  labs(y = expression("Nitrate (mg/L)")) +
