...
# n is the number of months we want the data to generate

linearinterpolation$x <- as.Date(linearinterpolation$x, origin = "1970-01-01")
# this fixes our x column. it changes it from # of days since 1970 to the actual date
...
> There is a significant decreasing trend of TP in October and November, but overall there is not a significant trend in TP.
...
**Autoregressive** models operate under the framework that given measurements is correlated with  previous measurements. For example, an AR1 formulation dictates that a measurement is dependent on the previous measurement, and the value can be predicted by quantifying the lag. 
