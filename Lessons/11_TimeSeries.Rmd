...
> No, because it doesn't take seasonality into account.
...
> If we want to look at something on a regular time scale that eliminates seasonality (like if we look at a specific date over the years), then it would be more approporiate.
...
# we can use the code above to see if there are any gaps in the data. It indicates that there's 1 missing date.
```
