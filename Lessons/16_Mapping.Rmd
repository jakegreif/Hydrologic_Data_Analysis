...
# end = 0.9 just stops the color pallet from going all the way through. We did it here because it ends with an ugly yellow
...
# %in% allows us to use the c() command. If we didn't have it, we would have to say filter(States == "GA,NC,SC" | States == "NC, and so on)
...
# the commented out code above can be used if we want to do clipping or other things
