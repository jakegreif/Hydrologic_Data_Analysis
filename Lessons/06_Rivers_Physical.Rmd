...
# put a negative in front of peak discharge to rank the lowest number as 1, highest as 30
...
# we are adding the intercept (coefficient 1) to the log of the recurrence interval times log of the year we want to know the discharge for
```
...
> The longer term reccurence intervals had a steeper slope, and the discharges were greater for the less frequent events. The shorter term reccurence intervals would cause a forecast to under-predict the likelihood of more extreme discharges.
...
> Use the most up-to-date data when calculating recurrence interval, and possibly incorporate some trend analyses as well. Another idea would be to use a moving window to calculate the recurrence intervals- which would essentially be like incorporating a trend analysis in it as well. 
...
SalFirst30Plot <- 
  ggplot(SaladoFirst30, aes(x = Year, y = PeakDischarge)) +
         geom_bar(stat = "identity") +
         xlab("Year")
print(SalFirst30Plot)

# Salado last 30 years
SaladoLast30 <- 
  SaladoDischarge %>%
  filter(Year > 1985) %>%
  group_by(Year) %>%
...
SalLast30Plot <- 
  ggplot(SaladoLast30, aes(x = Year, y = PeakDischarge)) +
         geom_bar(stat = "identity") +
         xlab("Year")
print(SalLast30Plot)

# First 30 model
SalF30RecurrencePlot <- 
  ggplot(SaladoFirst30, aes(x = RecurrenceInterval, y = PeakDischarge)) +
  geom_point() #+
  #scale_x_log10() 
print(SalF30RecurrencePlot)

Salado.RIF30model <- lm(data = SaladoFirst30, PeakDischarge ~ log(RecurrenceInterval))
summary(Salado.RIF30model)

Salado.RIF30model$coefficients[1] + Salado.RIF30model$coefficients[2]*log(100)
# 6584.689
Salado.RIF30model$coefficients[1] + Salado.RIF30model$coefficients[2]*log(500)
# 8981.524

# Last 30 model
SalL30RecurrencePlot <- 
  ggplot(SaladoLast30, aes(x = RecurrenceInterval, y = PeakDischarge)) +
  geom_point() #+
  #scale_x_log10() 
print(SalL30RecurrencePlot)

Salado.RIL30model <- lm(data = SaladoLast30, PeakDischarge ~ log(RecurrenceInterval))
summary(Salado.RIL30model)

Salado.RIL30model$coefficients[1] + Salado.RIL30model$coefficients[2]*log(100)
# 14431.95
Salado.RIL30model$coefficients[1] + Salado.RIL30model$coefficients[2]*log(500)
# 19808.72
```
