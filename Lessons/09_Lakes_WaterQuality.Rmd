...
options(scipen = 100)
# This is a work around to remove the E notations (if numbers are huge) in the data
...
# Here we're taking everything from LAGOSlocus and adding all the columns that match up in LAGOSstate by the state_zoneid
...
# creates a sampleyear and sample month column from the year and month in the sampledate column
         season = as.factor(quarter(sampledate, fiscal_start = 12))) %>%
# quarter function divides the data into quarters. Here we're saying starting at month 12, so 12, 1, and 2 will be our winter season, and so on. 
  drop_na(chla:secchi)
...
# this is a way to pick out colors from a colormap (picks out specific hex codes)
```
...
stateTSIjitter <- ggplot(LAGOStrophic, aes(x = state, y = TSI.chl, color = season)) +
  ...
  # These rectangles represent each trophic state
...
> Violin: Shows us info about the distributions, range, and median values, but it didn't quantify each state

> Bar: Shows us the number of measurements, and the distribution of those measurements.
